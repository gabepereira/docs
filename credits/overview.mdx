...
- `/odds/events` - Get upcoming events with odds
- `/odds/live-events` - Real-time live events
...
for (let i = 0; i < 100; i++) {
  await fetch(`https://api.mrdoge.co/odds/events/${eventIds[i]}`, {
    headers: { 'Authorization': 'Bearer sk_live_...' }
...
const response = await fetch(
  'https://api.mrdoge.co/odds/events?competitionId=premier-league&limit=100',
  { headers: { 'Authorization': 'Bearer sk_live_...' } }
); // 1 request = 1 credit!
```
</CodeGroup>
