...
<CodeGroup>
```bash cURL
curl -X GET "https://api.mrdoge.co/odds/events" \
  -H "Authorization: Bearer sk_live_your_api_key_here"
```

```javascript JavaScript
const response = await fetch("https://api.mrdoge.co/odds/events", {
  headers: {
...
response = requests.get('https://api.mrdoge.co/odds/events', headers=headers)
```

```php PHP
<?php
$ch = curl_init('https://api.mrdoge.co/odds/events');
...
| Standard events endpoints (`/odds/events`, `/odds/live-events`)| **1 credit**         |
...
fetch("https://api.mrdoge.co/odds/events", {
