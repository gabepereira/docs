...
<ResponseField name="homeTeam" type="object">
  Home team object with `id`, `caption`, and `regionId`
</ResponseField>
...
<ResponseField name="competition" type="object">
  Competition object with `id`, `caption`, `regionId`, and nested `region`
</ResponseField>
...
<CodeGroup>
```bash cURL
curl https://api.mrdoge.co/odds/events/evt_abc123
```

```javascript JavaScript
const response = await fetch('https://api.mrdoge.co/odds/events/evt_abc123');
...
response = requests.get('https://api.mrdoge.co/odds/events/evt_abc123')
event = response.json()
print(event)
```
</CodeGroup>
